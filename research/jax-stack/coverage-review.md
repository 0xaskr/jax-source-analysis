...
| 交付组 | 已有材料 | 仍需补齐 |
|---|---|---|
| 软件栈总览 | [组件与编译/Pallas 图](overview.md)，固定依赖、公开/私有边界；[LLVM/ORC](llvm-and-objects.md) 与 [CPU runtime](cpu-executable-and-trace.md) | 普通 TPU 的公开提交/完成/等待链；取得 U03 后关联目标 libtpu/LLO/运行证据 |
| matmul 源码与 API 索引 | [166 个入口、53 条关系](source-index.json)；四种程序变换；[逐 pass 导读](matmul-pass-walkthrough.md)、IR/LLVM/object/thunk；[10 单元 Notebook](matmul-lowering.ipynb) | 普通 TPU 与 Pallas 的目标编译产物和运行映射；分片传播入口目前只有简要索引，不能声称完整的分片图研究 |
...
|---|---|---|
| R01 组件/API | [overview](overview.md) 覆盖主要组件、依赖和 CPU/TPU 编译分支；CPU executable 与对象接口已有证据 | 普通 TPU 提交/完成/等待的公开调用链可继续源码研究；私有实现与真机需 U03 |
...
1. **可以现在做：普通 TPU 公开运行接口。** 沿 `PyLoadedExecutable::ExecuteSharded`、
   IFRT `PjRtLoadedExecutable::Execute`、PJRT C API 的执行/事件/等待接口，补充源码边、
   所有权与完成语义。只将实际读到的函数体作为 SOURCE-ONLY 证据。明确哪些进入 plugin，
   不以公共 wrapper 推断 libtpu 私有实现、TPU 耗时或实际执行线程。
