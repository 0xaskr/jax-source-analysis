...
两组 CPU 负对照及一次构建身份拒绝记录。**自定义 C++ 补丁尚未编译、加载或产生事件**。
补丁位置、生命周期和回滚步骤见 [pass-event-patch.md](pass-event-patch.md)。

## 当前实际证据

两组最新 capture 是 `artifacts/jax-stack/pass-events-unpatched-003` 与
...
5. 自定义事件须携带 pass、pipeline、module、program_id、status、changed；目标模块
...
当前 `source_build_verified` 和 `patched_runtime_pair_accepted` 均为 false。
成功自建 wheel 的加载和正对照分支仍待真实执行。脚本使用研究 capture 记录验收，不修改
...
```bash
.venv/bin/python -B research/jax-stack/pass_events_probe.py \
...
.venv/bin/python -B research/jax-stack/verify_pass_events.py \
...
增加 `--jaxlib-build-manifest manifests/build-fingerprints/kickoff-cpu-source-002.json`。
...
```bash
.venv/bin/python -B research/jax-stack/verify_pass_events.py --selftest
```
