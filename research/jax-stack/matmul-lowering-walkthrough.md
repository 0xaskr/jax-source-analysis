...
3. **`ApplyXlaTransforms`** — 出现在两处：`cpu_compiler.cc:1177-1178` 的
   ...
   [软件栈图](figures/overview-software-stack.svg) 里 D 接口（可写 HLO hook）的落点**，
...
（见 [CPU executable 与 trace](cpu-executable-and-trace.md)、
[LLVM 与对象](llvm-and-objects.md)）。
...
- 索引里的 `dot_general` 相关条目在换 pin 后仍未重新核对 `source_sha256`，
  见 [索引导读](source-index-digest.md) 顶部的漂移提示。

## 7. 与其它材料的关系

- [matmul 的实际 CPU pass 改写](matmul-pass-walkthrough.md)：已有 capture 上的**逐 pass** 审计，
  对应本文 2.9 的后半段，但基于换 pin 前的产物。
- [源码与 API 索引导读](source-index-digest.md) / [source-index.json](source-index.json)：
  按组件列出的入口清单；本文是沿**一条链路**的纵切。
- [软件栈与入口总览](overview.md)：组件与分支的全局视图。
