...
或加速比。结果见 [overlap-results.json](overlap-results.json)，原始 capture 为
...
各入口的 revision、文件指纹和调用位置已加入 [source-index.json](source-index.json)。
...
```bash
.venv/bin/python -B research/software-stack/overlap_probe.py \
  --output artifacts/jax-stack/overlap-cpu-new
.venv/bin/python -B research/software-stack/verify_overlap.py --selftest
```
