...
[external-descriptor-results.json](../tools/external-descriptor-results.json)；这份记录为 `SOURCE-ONLY`，
...
```bash
.venv/bin/python -B research/software-stack/tools/audit_external_descriptors.py \
  --output artifacts/jax-stack/external-descriptors-new
.venv/bin/python -B research/software-stack/tools/verify_external_descriptors.py \
