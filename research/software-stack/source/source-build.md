...
[Hack 验收](../compiler/pass-event-acceptance.md)；原始上游树未修改。
...
构建成功与运行门槛的记录见 [source-build-results.json](../tools/source-build-results.json)。
...
已独立验证 [build-dependency-results.json](../tools/build-dependency-results.json)，最新终态材料在
...
```bash
.venv/bin/python -B research/software-stack/tools/verify_build_dependencies.py \
