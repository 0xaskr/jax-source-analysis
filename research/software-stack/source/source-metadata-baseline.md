...
[source-metadata-results.json](../tools/source-metadata-results.json)。

这是对 [属性/cost](../compiler/attributes-and-cost.md) 和 [latency metadata](../performance/latency-model.md) 历史
实验的独立源码复验；旧 capture、旧结果与宿主机 wheel 均保留。
[属性 Notebook](../compiler/attributes-cost.ipynb) 的 8 个代码单元已执行：前六个保留旧 CPU 路径，
第七个复查这里的源码 capture 指纹、已保存身份及全部 16 组数值结果，最后一个复查
独立的 [原生 latency parser](../compiler/latency-parser-native.md) 结果。
...
[capture_runtime.py](../tools/capture_runtime.py)。完整 manifest、wheel、Git revision 与已加载
...
artifacts/jax-stack/source-runtime-002/baseline-env/venv/bin/python -B \
  research/software-stack/tools/source_runtime_suite.py --suite metadata \
