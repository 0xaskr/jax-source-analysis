...
对应 R04/R12。[准备脚本](../tools/prepare_pass_runtime.py) 已实际创建并运行独立 CPU 环境；
[结果](../tools/runtime-environment-results.json) 与 [验证器](../tools/verify_runtime_environment.py) 保留复制、
...
[pass-event-acceptance.md](../compiler/pass-event-acceptance.md)。此处仍为 `RUN-CPU + VERSION-SKEW`。
...
```bash
.venv/bin/python -B research/software-stack/tools/prepare_pass_runtime.py \
...
脚本不应用源码补丁，完整生命周期仍按 [pass-event-patch.md](../compiler/pass-event-patch.md) 执行。
...
```bash
.venv/bin/python -B research/software-stack/tools/verify_runtime_environment.py --selftest
```
...
[capture_runtime.py](../tools/capture_runtime.py) 复用成功构建 gate 与 native/wheel 字节核对。
...
已在保留的独立旧 wheel 环境实际执行并复查 [CPU 复验结果](../tools/cpu-revalidation-results.json)：
...
artifacts/jax-stack/source-runtime-002/baseline-env/venv/bin/python -B \
  research/software-stack/tools/matmul_probe.py \
  --output artifacts/jax-stack/cpu-matmul-source-new \
  --jaxlib-build-manifest manifests/build-fingerprints/kickoff-cpu-source-003.json
artifacts/jax-stack/source-runtime-002/baseline-env/venv/bin/python -B \
  research/software-stack/tools/verify_research.py --capture artifacts/jax-stack/cpu-matmul-source-new
```
