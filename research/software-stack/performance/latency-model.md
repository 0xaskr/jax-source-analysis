...
[latency-metadata-results.json](../tools/latency-metadata-results.json)，source contract 见
[latency-model-contract.json](../tools/latency-model-contract.json)。CPU capture 为
...
标签、拥有者及 CPU 成本观察一致，无 `VERSION-SKEW`，见 [源码复验](../source/source-metadata-baseline.md)。
随后已运行固定源码的 [原生 latency parser 测试](../compiler/latency-parser-native.md)：原测试和
...
```bash
.venv/bin/python -B research/software-stack/tools/latency_metadata_probe.py \
  --output artifacts/jax-stack/latency-metadata-new
.venv/bin/python -B research/software-stack/tools/verify_latency_metadata.py --selftest
```

交互实验并入现有 [属性/cost Notebook](../compiler/attributes-cost.ipynb)，无需另建业务模型。
