...
完整 **18 组 CPU 数值 / 3,898 个产物**均通过，见 [源码运行基线](../source/source-runtime-baseline.md)。
...
[attributes-and-cost.md](../compiler/attributes-and-cost.md)。已有 roofline 模块与修正位置见
[roofline.md](../profiling/roofline.md)，其中 XProf native converter 与目标 TPU 运行尚未验证。
...
[fusion-and-memory.md](../performance/fusion-and-memory.md)。这些结果不计为真实业务 split 或 TPU 内存验收。
...
[overlap-and-scheduling.md](../performance/overlap-and-scheduling.md)。实际 async pair 被 CPU 转回同步，
同步 `control_dep` 变成 dot→all-reduce 控制边。匹配源码复验后，显式 layout 的异步控制
错误变为 Host 未注册 `all-reduce-start`；其余三个 Python/MLIR 失败仍在。
[调度 Notebook](../performance/overlap-scheduling.ipynb) 的 4 个单元已真实执行，包含新的双 CPU capture。

LHS `latency_metadata` 的解析、模型选择与调度调用链已核对，见
[latency-model.md](../performance/latency-model.md)。8 个 CPU 标签/成本样本已在源码 wheel 复验。
两类实验共 14 组 metadata、2 组 HLO 改写执行、117 个产物；生产与复查进程均绑定
源码 native payload，旧结论在本例保持一致，见 [源码 metadata 基线](../source/source-metadata-baseline.md)。
...
[原生 parser 结果](../compiler/latency-parser-native.md)。这不增加 GPU consumer 或 TPU 执行证据。

CPU LLVM/对象代码/ORC 的选定接口已对应到运行产物，见 [LLVM 与对象](../compiler/llvm-and-objects.md)。
...
[编译事件补丁](../compiler/pass-event-patch.md) 已完成规范 Git diff、应用/反向应用及原始字节恢复。
无补丁源码基线里，默认和禁用 algsimp 的 generic 事件均为 3，与固定源码的 filter 前
TraceMe 位置一致；自定义事件为 0，warm 范围没有新编译事件。
[cold/warm/filter 验收脚本](../compiler/pass-event-acceptance.md) 要求补丁构建成功、实际 native 字节
...
[pass-hack-results.json](../tools/pass-hack-results.json)。这是 CPU 编译器 Hack 验收，目标 TPU
...
归档及解释器/共享库/头文件，467 个产物通过语义与反例验证，见 [构建输入记录](../source/build-inputs.md)。
该记录不把缓存条目当作实际目标依赖，不把构建后本机探测当作构建期进程采样。
匹配源码 matmul 的逐 pass 导读已完成当前样本审计，见 [matmul-pass-walkthrough.md](../../call-to-llo/matmul/matmul-pass-walkthrough.md)。
...
CPU executable 与运行时分支已进一步取证，见 [cpu-executable-and-trace.md](../runtime/cpu-executable-and-trace.md)。
...
原始 XSpace 关联已验证，见 [xspace-contexts.md](../profiling/xspace-contexts.md)：49 组一对一关联、
...
普通 TPU 的 [公开执行接口](../runtime/tpu-runtime-boundary.md) 已完成源码子项：新增 32 个入口、
24 条关系，分开 Python/C++ 提交分支、buffer/执行 status/effect 等待及 callback 生存期。
12 项结论、15 个源码检索条件和九个结构反例通过来源检查；没有新增设备运行。
[Shardy 往返与分区](../compiler/shardy-round-trip.md) 已补 21 个入口、15 条关系及双 CPU 实验。
...
[独立环境](../source/runtime-environment.md) 的旧 wheel 复验与新增源码 wheel 复验分别保留，
...
[业务合同与验收](../workload/sglang-v7-workload.md)。原等待输入状态保留为历史，不继续重复询问。
...
[profiling.md](../profiling/profiling.md)：host 生命周期、CPU 冷编译 pass 事件、Pallas 生产 lowering
生成的配对标记已验证。后者只是 CPU 主机上的开放源码 TPU lowering，不计为 TPU backend
编译或执行。两种 Pallas 解释路径对照见 [pallas-comparison.md](../pallas/pallas-comparison.md)。
