...
对应 R04/R12。可审查补丁为 [xla-hlo-pass-events.patch](../tools/xla-hlo-pass-events.patch)，
历史准备结果为 [pass-event-patch-results.json](../tools/pass-event-patch-results.json)，其证据仍为
`SOURCE-ONLY`。新增 [带测试的完整补丁](../tools/xla-hlo-pass-events-with-tests.patch) 已应用到
独立 XLA clone，25 个原生 C++ 测试全部通过，见 [首版测试结果](../tools/pass-event-cpp-results.json)。
实际运行发现导出器隐藏 `program_id`，因此最终使用
[可导出身份的补丁](../tools/xla-hlo-pass-events-exportable.patch)，25 个测试再次通过。
该补丁的 wheel 已实际加载：默认/过滤分别捕获 127/124 个自定义事件，数值与 warm
对照通过。源码已反向恢复；完整三状态验收见 [pass-hack-results.json](../tools/pass-hack-results.json)。
...
```bash
python3 -B research/software-stack/tools/prepare_cpp_test_dependency.py \
...
git -C artifacts/jax-stack/source-build-001/clones/xla apply --check \
  "$PWD/research/software-stack/tools/xla-hlo-pass-events-exportable.patch"
...
```text
--source-patch=xla=research/software-stack/tools/xla-hlo-pass-events-exportable.patch
```
...
```bash
.venv/bin/python -B research/software-stack/tools/prepare_pass_event_patch.py \
  --output artifacts/jax-stack/compiler-event-patch-new
.venv/bin/python -B research/software-stack/tools/verify_codegen_and_patch.py --selftest
```
