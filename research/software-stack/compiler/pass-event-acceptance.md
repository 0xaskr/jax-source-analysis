...
对应 R04/R12。[生产脚本](../tools/pass_events_probe.py)、[事件规则](../tools/pass_event_checks.py) 和
[验证器](../tools/verify_pass_events.py) 已实际运行；[结果清单](../tools/pass-events-results.json) 保存
历史 CPU 负对照及一次构建身份拒绝记录。最终补丁的实际构建、加载及回滚证据另保存在
[pass-hack-results.json](../tools/pass-hack-results.json)，统一复查入口是 [verify_pass_hack.py](../tools/verify_pass_hack.py)。
...
```bash
.venv/bin/python -B research/software-stack/tools/pass_events_probe.py \
...
.venv/bin/python -B research/software-stack/tools/verify_pass_events.py \
...
```bash
.venv/bin/python -B research/software-stack/tools/verify_pass_hack.py --selftest
```

默认复查已发布证据并运行 selftest：

```bash
.venv/bin/python -B research/software-stack/tools/verify_pass_events.py --selftest
```
...
[Pallas/profiling Notebook](../profiling/pallas-profiling.ipynb) 包含不可变 capture 复查和新进程重跑。
