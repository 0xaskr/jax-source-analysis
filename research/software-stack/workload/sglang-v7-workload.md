...
1. **预检**：[Falcon 配置](../../../deploy/falcon/sglang-v7-preflight.yaml) 内嵌
   [生产脚本](../tools/sglang_v7_preflight.py)。运行最多 30 分钟；确认八 device/四 chip，八设备普通
...
预检结果、失败与真机资源状态在 [status.json](../history/status.json) 记录。原始 captures 和集群返回
保留于忽略的 artifacts；只发布必要且脱敏的实验标识、哈希和结果。

## 当前实验结果与入口兼容性

[预检结果](../tools/sglang-v7-preflight-results.json) 已取得真实 TPU7x 身份：4 组物理坐标、每组两个
...
[外部基线驱动](../tools/sglang_v7_baseline.py) 是为前两项写的兼容包装，但它在 decode 分支使用了
尚未赋值的 `ids_cpu`，**从未跑通过 decode 循环**，且缺少原生 benchmark 各调用点都有的
`process_allgather`。因此改用 [原生路径驱动](../tools/sglang_v7_driver.py)：直接调用 benchmark
...
0.11.1，见 [交接文档 4.1](../history/AGENT-HANDOFF-2026-09-15.md)）；
[设备采集脚本](../tools/tpu_device_events_probe.py) 在 fresh process 做默认/开启对照，
[raw XSpace 验证器](../tools/verify_tpu_device_capture.py) 区分 module/op/TraceMe、整型 ps、
