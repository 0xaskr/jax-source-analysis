...
| matmul 源码与 API 索引 | [219 个入口、92 条关系](source-index.json)；四种程序变换；[逐 pass 导读](../call-to-llo/matmul-pass-walkthrough.md)、IR/LLVM/object/thunk；[10 单元 Notebook](../call-to-llo/matmul-lowering.ipynb) | [Shardy 往返/分区](shardy-round-trip.md) 已补双 CPU 运行；普通 TPU 与 Pallas 的目标编译产物和运行映射仍缺 U03 |
...
| R02 lowering | [源码 003](source-runtime-baseline.md) 四组 matmul 数值与重载；[640 个 pass 边界](../call-to-llo/matmul-pass-walkthrough.md)、22 组叶子改写；LLVM/object/thunk/runtime 对应 | TPU 目标后端产物、LLO 与设备映射需 U03；不把 CPU emitter 解释为 TPU 实现 |
