...
[source-runtime-results.json](source-runtime-results.json)。宿主机 `.venv` 仍保留旧 wheel。
...
[source_runtime_suite.py](source_runtime_suite.py) 依次采集并验证三个主题；某项失败时
...
artifacts/jax-stack/source-runtime-002/baseline-env/venv/bin/python -B \
  research/software-stack/source_runtime_suite.py \
...
见 [完整 Hack 验收](pass-event-acceptance.md)。原始上游树和当前独立 clone 均干净。

这份基线不增加 TPU 编译、TPU 运行、LLO、设备时间或真实推理模型的证据；完整 kickoff
及 U01–U03 仍按 [PLAN.md](PLAN.md) 与 [status.json](status.json) 继续。
