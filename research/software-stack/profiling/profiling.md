...
及三状态对照，见 [pass 事件验收](../compiler/pass-event-acceptance.md)；TPU 真机事件仍未验收。
结果见 [extension-results.json](../tools/extension-results.json)，独立复查入口为
[verify_extensions.py](../tools/verify_extensions.py)。该历史 capture 保留 `VERSION-SKEW`；新增
...
在 `finally` 中 `close()`，完整示例见 [profile_events_probe.py](../tools/profile_events_probe.py)。
...
[普通 TPU 公开运行接口](../runtime/tpu-runtime-boundary.md) 进一步区分了输出 buffer ready、执行
...
[compiler_events_probe.py](../tools/compiler_events_probe.py) 关闭持久编译缓存，分别标记
...
固定源码构建、加载、数值对照和回滚均已完成，见 [编译器 Hack](../compiler/pass-event-acceptance.md)。
上面的历史 wheel 捕获保留原证据边界。Python `jit` 函数体里的标记随 tracing 执行，
不代表 executable 每次执行的内部区间。

## 设备内部：Pallas 名称栈变成 IR 操作

[mosaic_events_probe.py](../tools/mosaic_events_probe.py) 使用 `A[16,128] @ W[128,128]`，
...
```bash
.venv/bin/python -B research/software-stack/tools/verify_extensions.py --selftest
.venv/bin/python -B research/software-stack/tools/profile_events_probe.py --output artifacts/jax-stack/profile-events-replay
.venv/bin/python -B research/software-stack/tools/compiler_events_probe.py --output artifacts/jax-stack/compiler-events-replay
.venv/bin/python -B research/software-stack/tools/mosaic_events_probe.py --output artifacts/jax-stack/mosaic-events-replay
```
