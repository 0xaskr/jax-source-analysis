...
[cpu-thunk-results.json](../tools/cpu-thunk-results.json) 保存完整复查结果。原始 matmul
...
[cpu_thunk_probe.py](../tools/cpu_thunk_probe.py) 对每个样本先编译并序列化，再 warm 一次；
...
一般化为异步操作完整时长。后续 [原始 XSpace 审计](../profiling/xspace-contexts.md) 已用 context 字段复查 33 对 thunk
...
[verify_cpu_thunks.py](../tools/verify_cpu_thunks.py) 重读原 capture、新运行、schema、完整清单、
...
schema 通过 [prepare_cpu_thunk_schema.py](../tools/prepare_cpu_thunk_schema.py) 在新目录重建。
...
```bash
"$PY" -B research/software-stack/tools/cpu_thunk_probe.py \
...
"$PY" -B research/software-stack/tools/audit_cpu_thunks.py \
  --runtime-capture artifacts/jax-stack/cpu-thunk-runtime-new \
  --output artifacts/jax-stack/cpu-thunk-audit-new
"$PY" -B research/software-stack/tools/verify_cpu_thunks.py \
