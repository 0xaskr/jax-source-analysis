...
[source-index.json](../tools/source-index.json) 的完整 revision、路径、行号与文件指纹约束。
本次增加 32 个入口、24 条关系；[runtime-chain-results.json](../tools/runtime-chain-results.json)
保存 12 项结论和检索保护条件。没有新增 CPU/TPU 编译或运行；已有 CPU trace 的证据
继续按 [CPU executable](cpu-executable-and-trace.md) 和 [XSpace](../profiling/xspace-contexts.md) 阅读。
...
[XSpace matcher](../profiling/xspace-contexts.md) 只覆盖 type 0/15，49 组已验证关联没有包含这里的
...
```bash
.venv/bin/python -B research/software-stack/tools/build_source_index.py
.venv/bin/python -B research/software-stack/tools/audit_runtime_chain.py --selftest --check-saved
.venv/bin/python -B research/software-stack/tools/verify_research.py
```
...
本节完成公开接口子项，完整范围继续见 [覆盖复查](../history/coverage-review.md) 和 [PLAN](../history/PLAN.md)。
