...
这些记录的 CPU `DotThunk`、LLVM 对象和 pass 结果不是 TPU LLO。继续这条链需要固定 `jnp.matmul` 输入、目标 TPU、JAX/jaxlib/libtpu 版本及同一次编译标识，然后保存 TPU HLO、后端阶段产物与 LLO。软件栈整体的源码入口见[开放探索索引](../../software-stack/index.md)。
