...
[源码运行基线](../../software-stack/source/source-runtime-baseline.md)。本次 pass 审计没有重新执行 HLO，也没有 TPU/LLO 证据。
后续 [CPU executable 与 trace](../../software-stack/runtime/cpu-executable-and-trace.md) 已单独验证剩余 dot 的 DotThunk/Eigen 分支。

原始 capture：`artifacts/jax-stack/source-runtime-002/suite/matmul/`。
派生记录：`artifacts/jax-stack/source-lowering-audit-001/passes/`；
[复查结果](../../software-stack/tools/pass-transition-results.json) 登记 59 个派生产物。
...
[匹配源码的对象结果](../../software-stack/tools/source-codegen-results.json) 和 [LLVM 导读](../../software-stack/compiler/llvm-and-objects.md)。
梯度中剩余的 dot 则由
...
以下命令在[固定镜像与源码 003 环境](../../software-stack/source/source-runtime-baseline.md)中运行；`PY` 指该独立
...
```bash
"$PY" -B research/software-stack/tools/audit_pass_transitions.py \
  --output artifacts/jax-stack/source-pass-audit-new
"$PY" -B research/software-stack/tools/verify_pass_transitions.py --selftest
"$PY" -B research/software-stack/tools/verify_codegen_and_patch.py --codegen-only \
...
[Shardy 往返与分区](../../software-stack/compiler/shardy-round-trip.md) 重新核对了本章四组单 CPU 的 `shardy-xla`
